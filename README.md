...
## 20. Update: Trending, Koleksi Cepat, Riwayat Download

Nggak perlu update rules — Trending & Koleksi Cepat murni hitungan dari data yang udah ada, Riwayat Download murni localStorage (sama kayak Baru Dilihat/Favorit).

- **Trending Preset** — shelf baru paling atas beranda, preset bernomor 1-6. **Jujur soal ini:** bukan trending "asli" (butuh data lonjakan unduhan per waktu yang nggak ada), tapi skor gabungan unduhan dibagi umur preset (`unduhan ÷ √umur_hari`, plus bonus 1.5x kalau umurnya di bawah 30 hari) — pola ranking klasik ala Reddit/HN, biar preset baru yang lagi naik daun nggak ketimbun sama yang lama tapi total unduhannya udah tinggi.
- **Koleksi Cepat** — section baru di sidebar, isinya 5 tag paling sering dipakai sebagai pintasan filter. Klik satu → langsung ke Beranda dengan filter tag itu aktif. Bukan folder buatan pengguna kayak di gambar referensi (itu butuh model data baru) — ini manfaatin tag yang udah ada.
- **Riwayat Download** — halaman baru di sidebar, nyatet preset yang pernah didownload di perangkat ini (localStorage, nggak ada batas jumlah, preset yang didownload ulang naik lagi ke atas). Ada tombol "Bersihkan" buat reset riwayatnya (nggak ngurangin hitungan unduhan di presetnya).

## 21. Update: Assets (Font/Audio) di detail preset

Nggak perlu update rules — field baru di preset yang sama (`assets`).

Form Tambah/Edit Preset sekarang ada editor "Assets (Font/Audio)" — sama pola-nya kayak editor link download (bisa nambah beberapa baris, masing-masing punya nama file, tipe (Font/Audio/Lainnya), dan URL download). Kalau diisi, detail preset nampilin section "Assets yang dibutuhkan" dengan ikon per tipe + tombol download langsung. Section-nya sembunyi otomatis kalau presetnya nggak punya asset.

## 22. Update: Tab Deskripsi/Tutorial/Komentar/Changelog

⚠️ **Update `firestore.rules` lagi** (collection baru: `presetComments`).

⚠️ **Ada index Firestore baru yang wajib dibuat** (sama caranya kayak index Pesan di Update 14): buka detail preset apa aja → klik tab **Komentar** sekali → kalau muncul error di toast, buka **Console browser (F12)** → ada link ke Firebase Console buat bikin index-nya → klik **Create Index** → tunggu sampai "Enabled". Query komentar itu gabungan filter `presetId` + urutan `createdAt`, jadi butuh index komposit.

Detail preset sekarang punya 4 tab:
- **Deskripsi** — sama kayak sebelumnya.
- **Tutorial** — field baru di form admin (opsional), cara pakai preset ini.
- **Komentar** — publik boleh baca tanpa login, tapi nulis komentar butuh Login Google. Komentar bisa dihapus sama penulisnya sendiri atau admin (moderasi). Nggak bisa diedit setelah terkirim — cuma bisa dihapus & ditulis ulang.
- **Changelog** — form admin punya editor "+ Tambah Catatan", tiap catatan otomatis dapet tanggal pas ditambahkan (nggak bisa diubah-ubah, sifatnya kayak log).

## 23. Update: Activity Feed

Nggak perlu update rules — field baru di preset (`updatedAt`), murni data yang udah publik.
